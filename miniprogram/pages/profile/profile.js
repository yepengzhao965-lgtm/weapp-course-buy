// This is a local copy of the original profile page logic from the user's repository.
// It initializes user data, handles phone number authorization and WeChat profile
// authorization, and exposes functions to update and re‑authorize user info.
...
Page({
  ...
  async onGetPhoneNumber(e) {
    ...
    try {
      ...
      const db = wx.cloud.database();
      // Insert a new user record if it doesn't exist, otherwise update the phone number
      await db
        .collection('users')
  ...
  // 微信授权：通过 getUserProfile 获取头像昵称并写入 users（docId = openid）
  onGetUserProfile() {
    wx.getUserProfile({
      desc: '用于完善用户资料',
      success: async (res) => {
      ...
      await db
        ...
        .catch(async () => {
          await db
            .collection('users')
            .doc(this.data.openid)
            .set({ data: { ...data, role: 'visitor', createdAt: new Date() } });
        });
      const got = await db
        .collection('users')
        .doc(this.data.openid)
  ...
  goAddCourse() {
    wx.navigateTo({ url: '/pages/add-course/add-course' });
  },
});